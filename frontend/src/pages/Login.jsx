...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    e?.preventDefault();
    setSubmitted(true);
    if (emailErr || passwordErr || !email || password.length < 8) return;
    setLoading(true);
    setError(null);
  ...
  return (
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <div className="rounded-2xl border border-border overflow-hidden bg-bg flex w-full max-w-215 min-h-160 shadow-[0_20px_60px_-30px_rgba(0,0,0,0.8)]">
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            <div className="mt-auto">
              <p className="text-[22px] leading-snug tracking-tight text-ink font-medium">
                "Cambiamos cuatro herramientas por TaskFlow. Ahora todo el equipo ve el mismo tablero."
              </p>
              <div className="mt-6 flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-linear-to-br from-indigo-500 to-[#7c3aed] grid place-items-center text-white font-semibold text-[13px]">MR</div>
                <div>
                  <div className="text-[13px] font-medium text-ink">María Reyes</div>
                  <div className="text-[11.5px] text-mute">Head of Product · Lumen</div>
                </div>
              </div>
              <div className="mt-8 flex items-center gap-2 text-[11.5px] text-mute">
                <span className="w-1.5 h-1.5 rounded-full bg-[#34d399]" />
                Más de 12k equipos confían en TaskFlow
              </div>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col">
          <div className="h-14 px-6 flex items-center justify-between border-b border-border">
            ...
            <div className="text-[12.5px] text-mute">
              ...
              <Link to="/register" className="text-indigo-400 hover:text-indigo-500 font-medium">Crear cuenta</Link>
            </div>
          </div>

          <div className="flex-1 flex items-center justify-center p-6">
            <form onSubmit={handleSubmit} className="w-full max-w-100">
              <h1 className="text-[26px] font-semibold tracking-tight text-ink">Bienvenido de vuelta</h1>
              <p className="text-[13.5px] text-mute mt-1.5">Inicia sesión para continuar con tus proyectos.</p>
              ...
              <div className="mt-7 flex flex-col gap-3.5">
                ...
                <button type="submit" disabled={loading}
                  className="mt-2 w-full h-12 rounded-lg bg-indigo-500 hover:bg-indigo-600 disabled:opacity-60 text-white font-medium text-[15px] transition-colors">
                  {loading ? 'Iniciando sesión…' : 'Iniciar sesión'}
                </button>
              </div>

              <p className="mt-8 text-[11.5px] text-mute text-center">
                Al continuar aceptas los <span className="text-ink/80 underline-offset-2 hover:underline cursor-default">Términos</span> y la <span className="text-ink/80 underline-offset-2 hover:underline cursor-default">Política de privacidad</span>.
              </p>
            </form>
          </div>
        </main>
